...
Components −512 in progress; NPC remains). Then PICKED **📱 TOUCH/MOBILE INPUT** — the §0 hard-frame gap (the game was UNPLAYABLE on its stated iPad/mobile target: zero touch handlers, Pointer-Lock-gated which iOS lacks). A grounded 3-lens design-gate workflow → committed spec (dual-zone floating-joystick + drag-look + crosshair; reuses the intent-SoT/verb-router/active-gate seams with ZERO movement-consumer change; M0→M3 ladder) + **TOUCH M0 ✅ + M1 ✅ SHIPPED (iters 128-133; M1 adversarially reviewed-clean)** — M0 the pure `touchMath.js` (joystick→booleans 8-way no-opposing-pair · PLC-verbatim look-clamp · multi-touch id→zone) + M1 the producer-wiring overlay (`TouchControls.jsx` capture-null + desktop-inert; reuses setIntent/performVerb/setActive; Terrain highlight + App-pause touch guards). Next: touch M2 (the visible thumb-cluster surface + the `mobile.png` fixture). Deferred behind it: S3-M5 part 3 (Player-loop SM-wiring) · world new-blocks (M4b+M5b) · S3-M6 NPC. Live scale (2026-06-13 post-touch-M1):
~25k LOC src / ~209 files / 990 unit tests (127 files) / 35 static-gate files / 17-state visual gate (LIVE-verified,
...
| **S3 engine + platform** | De-monolith god-files (characterization-first) · partial-ECS resolution · **touch/mobile input + UI** (the third-person control pivot likely fuses here — Kevin 2026-06-07) · real perf numbers + adaptive-quality hardening · gen-system perf | **LIVE — the entry gate is MET (all 4 Aspects ✅, 2026-06-11), so the recorded "AFTER S2-B complete" condition is satisfied (the ≥3-Aspect relaxation was never needed).** De-monolith M1-M4 + M5 parts 1-2 shipped; **touch/mobile input DESIGN-GATED + M0+M1 ✅ shipped (iters 128-133, M1 review-clean) — M2+ in flight** (the §0 hard-frame gap: the game was unplayable on iPad/mobile; now wired to be touch-playable, M2 adds the visible surface); perf-number hardening + partial-ECS resolution still queued. Backlog in ROADMAP §S3-deferred. |

...
- **Weather (S1+S2):** rain "only in one spot / vanishes on turn" = the 3 weather instancedMeshes lacked `frustumCulled={false}` (THREE caches the bounding sphere once; stale sphere mis-culls moving particles) — fixed `f0dd78c`. Snow "never shows" = a global clear→rain→snow timer that rarely matched the local biome; rebuilt to `clear↔storm` where the player's CURRENT biome decides precip (cold→snow/temperate→rain/desert→dry) via a new pure `precipFor()` — `5d3e10b`, TDD'd.
- **Spell-VFX (S3):** a background research Workflow (3 lanes: shipped-game taste / web-three technique / anti-childish code-ground) → approved spec `specs/2026-06-29-crafty-spell-vfx-sota-redesign.md` + plan → 7 gated geometry-first slices. Root cause: all 4 shared one ball+core+glow+sin-pulse recipe → all bloomed to the same pink/white blob with symmetric platonic primitives. Each element is now distinct: **fire** = upward flame teardrop + blackbody ramp (`spellGeometry.buildFireTeardrop`); **ice** = solid faceted crystal cluster, NON-additive + inverted-hull edge-bloom rim (`buildIceShards`) — the worst offender (was an additive glow-ball) fixed; **lightning** = thin jagged additive white-violet wire + core filament; **arcane** = layered orbital rune-wheel + magenta/cyan duotone. Plus a shared per-element motion grammar (`spellMotion.js`: roil/static/strobe/orbit) + palette lane-separation (resolved the ice/lightning blue collision) + a capture extension (`spell-iceball/lightning/arcane.png`, gate 21→24 states). Commits `5e7ca63`·`d56269a`·`0066bfc`·`d0d116a`·`d114bf4`·`b062ac0`·`962dd38`·`0f8b22a`. unit 1896→1936, build+eslint green throughout; each element eyeballed on its rendered cast frame.
- **Deferred (Kevin live-eye / taste-gated, surfaced in KEVIN-REVIEW):** S3.8 secondary-motion pools + S3.9 impact/telegraph timing + Phase-2 shaders (true ice fresnel, fire heat-haze) + biome ground-tint shader wiring. Owed: a clean complete:true 24-state capture (title-mascot env-timeout blocked it all session).

### June 29, 2026 — 🏁 v6 DE-MONOLITH COMPLETE (A3 residual end-state reached) — all autonomously-safe v6 work done
- **A3 Components reached its documented residual end-state.** A3.1 extracted PositionTracker → `src/systems/` (`ff3b818`; 1345→1330). Verification then established that Components.jsx is a SINGLE component — the `Player` useFrame imperative controller (lines 77-1328) — which **already delegates all pure logic to 32 imported `game/*` modules** (locomotion, beasts/transform, ferocity, dodge, cameraKick, trauma, all four Aspect channels, gameFeel, cooldownMirror, spawnPlacement, …, mostly from prior S3/Aspect-spine work). Its only inline functions are event handlers + a rapier collider predicate — coupled to refs/store/rapier, not pure. The remaining ~1330 LOC is the irreducibly-imperative Player loop + input wiring, which the plan + decisions-of-record require NOT be split (Game-Loop-Isolation / ordering risk). **Components stays a single cohesive controller delegating to pure modules — no longer an accidental god-file; this is the acceptable A3 end-state.**
- **🏁 THE v6 DE-MONOLITH IS COMPLETE.** Across this run the autonomous loop dissolved **2 of the original 5 god-files entirely** — SimplifiedNPCSystem 934→183 (7 ECS systems + a `_npcShared` seam → `src/systems/`) and GameScene 933→304 (6 render drivers + a scratch seam → `src/render/`) — extracted PositionTracker from the 3rd, and verified the Components Player loop as the documented irreducible residual. ~17 individually-gated extraction slices, every gate green throughout (unit held/grew at 1896, build + eslint clean, visual 22/22 on every render-affecting slice). Combined with **Phase C** (all autonomous tech-debt: visual-gate fail-loud, E2E perf fps-liveness + heap tripwire, coherence-gate verify), **all autonomously-safe v6 work is now done.** The remaining Crafty backlog is entirely Kevin-gated (zh-CN i18n, S4, control-scheme #9, compass #6, touch radial wheel, mob/boss art, clip/photo-mode, live-eye taste, affixes wiring, coherence cut-gate) — surfaced to Kevin for the next directive.

### June 29, 2026 — Phase A2 de-monolith: GameScene dissolved to a thin shell (933 → 304 LOC, −67%)
- **The 933-LOC GameScene god-file is now a thin Canvas/lights/postproc/composition shell.** Extracted its 6 render drivers + a shared scratch seam into `src/render/`, one individually-gated slice each (verbatim, behavior frozen): A2.1 Sun (`e4656d1`), A2.2 MoodGradeDriver (`0df1f4a`), A2.3 BloomSpikeDriver (`70f06db`), A2.4 PointerLook (`0552f37`), A2.0 `_sceneScratch` hoist (`3343f49`), A2.6 SpatialAudioController (`edd918a`), A2.5 WeatherSystem. GameScene.jsx ≈ 304 LOC. **Of the original 5 god-files, 4 are now fully dissolved (Components is the last, A3 in progress, 1345→1330).**
- **Render-driver vs pixel-irrelevant gating proven:** drivers that render (Sun/Bloom/Mood/Weather) each required a GREEN 22/22 capture; returns-null/audio-only ones (PointerLook/SpatialAudio) + the const/seam hoists accept the "all scene frames + zero render crashes" signal under load. Capture-infra load-spike retries handled throughout. Each slice removed its now-dead host imports (eslint doesn't flag those; knip is the ratchet) — GameScene's import header shrank from ~12 heavy deps to a thin set.

### June 29, 2026 — Loop RE-ARMED v6 = TECH-DEBT → DE-MONOLITH (Kevin steer)
- **Kevin re-armed the autonomous loop** ("do all the autonomous-doable tech-debt first. and then the de-monolith. … keep going autonomously"). Charter §0 + kernel prompt → **v6**; ACTIVE_PLAN cursor rewritten; de-monolith elevated from Kevin-gated scope/taste to full loop authority (`a5a0b6e`).
- **C1 — visual-gate FAIL-LOUD hardening (`f14ed1b`, KEVIN-REVIEW item #12).** Capture already aborts the diff on a render-crash (non-zero exit); the residual hole was an *isolated* `diff.test.js` run passing silent-green on a STALE `current/` left by a crashed/timed-out capture (the iter-105 mount-crash class; the 2026-06-28 heavy-scene capture timeout). FIX: `capture.mjs` writes a `current/.capture-meta.json` sentinel (complete:false at start, complete:true only at a clean crash-free end) + a pure `src/devtest/captureFreshness.js` predicate (9 unit tests, verifiable even when puppeteer is flaky) + a freshness assertion in `diff.test.js`. Verified: visual gate **22/22** on a fresh capture; the legitimate diff-alone-of-prior-good-capture path still passes.
- **C2a — E2E perf coverage, frame-rate-under-load.** Added a DEV-only `?perfsec` fast-window override to the existing perf-probe harness (pure `parsePerfParams`, 6 unit tests) so the night-siege probe runs a short window; `tests/e2e/perf-siege.spec.js` drives scenario B (deep-night, nightCount=6 saturated spawn ramp) and asserts UNDER-LOAD LIVENESS — the rAF loop ran the full window, sampled real frames, threw no fatal runtime error. Honest swiftshader metric (absolute FPS is GPU/load-dependent). Verified **1 passed (1.6m)**. Unit **1881 → 1896**, build + eslint green.
- **C2b/C3 — perf memory tripwire + coherence-gate verify (`230eb1c`).** Probe forces a full GC (`--js-flags=--expose-gc`) around the siege + records heapGrowthMB; perf-siege asserts a catastrophic-leak tripwire (<50MB; measured 0). Coherence VERIFIED (not built): knip dead-exports = 0; the destructive cut-gate correctly stays PARKED (auto-building it is Kevin-gated + goal-frame-risky) — surfaced to KEVIN-REVIEW. **Phase C (all autonomous tech-debt) COMPLETE.**

### June 29, 2026 — Phase A de-monolith: SimplifiedNPCSystem fully dissolved (934 → 183 LOC, −80%)
- **The 934-LOC NPC god-file is now a thin orchestrator.** Per `docs/superpowers/plans/2026-06-29-crafty-de-monolith.md`, extracted its 7 independent ECS systems + a shared seam into `src/systems/`, one individually-gated slice each (all verbatim moves, behavior frozen, pixel-identical): A1.1 MinimapSyncSystem (`ebf492a`), A1.2 EnemyProjectileSystem (`780bb15`), A1.3 SpawnerSystem (`11b5637`), A1.4 AIWorkerSystem (`f1f7f4d`), A1.5 `_npcShared` seam-hoist [xpOrbsQuery/lootDropsQuery/useEntities/nextSpawnId/spawnLootDrop] (`72a9f4a`), A1.6 XPOrbSystem + A1.7 LootSystem (`0e1ca6a`), A1.8 CombatSystem. SimplifiedNPCSystem.jsx is now ~183 LOC (the NPCSystem orchestrator + a getItemRarity re-export). A1.9 (relocate that re-export) skipped as zero-value churn.
- **Method that held across 8 slices:** read → grep host-unused vs shared imports → verbatim move with `../`-relative paths → remove the def + dead host imports → re-run unit → retarget structural gates that string-match the moved code by *concatenating* the extracted file into their source read (never weaken/delete; widen hardcoded `./`-import regexes to `\.\.?/`) → build + eslint + capture. Every slice green (unit held at 1896; visual 22/22 when load permitted, else the documented "all scene frames + zero render crashes" signal for capture-suppressed NPC systems). Architecture-reality line in `.agent/AGENTS.md` updated: **2 god-files left (Components 1345, GameScene 933).**

### June 28–29, 2026 — POST-AUDIT testing/perf campaign COMPLETE → loop stopped by Kevin for a manual deep-dive
- **The 2026-06-28 audit's #1 gap (no E2E / integration / visual testing on features) is substantially closed.** Autonomous /loop ran ~40 committed units, all gates green throughout, no AI footers. Unit suite **1756 → 1881 (+125)**.
  - **3 confirmed-HIGH bug fixes** (earlier in session): HUD health-bar wrong-context-key (`7b6e7b7`), melee dropped id+position → dead threat-arrow/SFX (`c01e244`), block-debris rapier-2.2 API → threw on every mine (`a36384e`). Plus win-state persistence (`114e5a6`), iceball freeze (`91fbc38`), F=cast/T=melee binding (`74fd858`).
  - **Vein A — DOM-panel integration tests (jsdom + @testing-library):** TradingInterface, Inventory (consume + attribute-allocate), SpellUpgradePanel talent-tree, CraftingTable — real render → real click → real store effect.
  - **Vein B — e2e gameplay-flow specs (Playwright dev-bridge + forcePlay):** day/night + dawn-reward, beast-form lifecycle, mana economy, soft-death protections, hunger/starvation, equip round-trip — now **10 spec files / 21 test cases**.
  - **Vein E — 13 pure-module unit suites:** lootTier, lowHealth, locomotion, cooldownMirror, dayPhase, spawnPlacement, invNormalize, compass, spellCast, npcRoutine, settingsPersist, mobFeatures, deniedReason.
  - **Vein C — render-perf hoists (capture-verified byte-identical):** audio-occlusion ray scratch (`485fa95`), ocean gerstner per-wave precompute (`4bc981f`); all other hot loops verified already-optimized.
  - **affixes #8 RESOLVED → KEPT** (tested loot-depth foundation; full wiring is a future balance milestone, not dead code).
- **Visual-gate infra outage isolated + recovered.** `npm run visual:capture` failed all session with a puppeteer CDP `Runtime.callFunctionOn timed out` (software-WebGL capture under machine load); reproduced on a clean tree (≈9×) → proven environment, not our code; surfaced to Kevin, never used to weaken a gate. **Kevin's Chrome restart fixed it** — re-ran → capture clean + **21/21 pixelmatch green**, retroactively confirming the session's commits moved zero pixels.
- **Loop STOPPED by Kevin (2026-06-29)** for a manual deep-dive on the remaining punted/incomplete/blocking items (see `memory/ACTIVE_PLAN.md` top banner "DEEP-DIVE QUEUE" + `docs/superpowers/KEVIN-REVIEW.html`). Compaction prep + `pre-compact-flush-kz` done. HEAD `8688601`, 1881 unit / 21 visual / build + eslint green.

### June 22, 2026 — Phase B build-out COMPLETE (B1–B7 all shipped); B5 mob-art slice 1 (`5724619`)
- **B5 mob art (bounded slice 1): humanoid arms.** Verify-before-assert found the boss + most mobs are ALREADY silhouetted (boss DRAGON_FEATURES horns/tail/spine + wings; mobs' per-type mobFeatures horns/ribs/shoulders/crests) — not raw box-stacks. The clearest remaining tell was upright HUMANOIDS lacking arms. Added `humanoidArms()` to the pure mobFeatures system: zombie reaches forward (shamble), villager hangs plain, skeleton gets bony arms beside its ribs. Derived from body dims, static (capture-safe), rendered via the existing features.map path. Structure unit-locked (>=2 arms/humanoid) + build/21-visual green; no humanoid close-up capture exists so the arm LOOK is flagged for Kevin. Further mob-art polish (proportions/animated limbs/animal snouts) is taste/Kevin-directed. 1728 unit.
- **🏁 ALL PHASE-B AUTONOMOUS BUILD-OUT DONE this session (Kevin "you pick, build all except B8"):** B6 humble loadout (`fc26af1`) + panel-capture determinism fix · B2 spell-color unify (`16a69f9`) · B7 wand/crystal economy (`53c7478`) · B4 quest rewards (`728635c`) · B3 shrine model (`860ddd5`) · B1 jungle vine-canopy (`e60b77a`) · B5 humanoid arms (`5724619`). Each TDD + chained-gate-blocked + pushed. **These form a coherent loop:** humble start (B6) -> earn crystals/gear through play -> wands cut spell mana (B7), quests grant gear (B4 ties to B6) -> distinct world to explore (B1 biomes, B3 shrine) -> clearer creatures (B5). B8 (multiplayer/monetization) + held items (R1 debris, I1 serena, I2 cloud reviewer, I3 dep-bump, F1–F5 taste) PARKED per Kevin.
- **Now: idle-hold for Kevin.** Flagged LIVE-LOOK eye-checks (headless can't judge taste): the B3 shrine silhouette · per-biome flora incl. B1 jungle · B5 humanoid arms · (standing) B2 spell colors in play. Plus the parked decisions await his steer.
...
- Locked by `tests/gates/spell-color-unify-gates.test.js` (12). Gate stayed green WITHOUT re-baseline — the unify is sub-6% on every captured frame (the dominant VFX core/glow was already spec-aligned; only trail/telegraph/spark/wand/mascot identity shifted). LIVE-LOOK of spell-cast confirms a coherent per-element identity (fire = one warm-orange family across telegraph/glow/trail/spark). 1697 unit / 21 visual / build + eslint.
- **Queue remaining:** B7 wand/crystal economy · B4 quest rewards · B3 shrine model · B1 flora slice-3 shapes · B5 mob/boss art. (B8 parked.)

### June 22, 2026 — Phase B build-out (Kevin: "you pick, build all except B8"); B6 shipped
- **Kevin's decision (2026-06-22):** for the KEVIN-REVIEW §1 prioritization, "you pick — build all except B8 (multiplayer/monetization); leave B8 + the other decision items for later." The loop now autonomously builds B1–B7 (sequenced by EV+verifiability), B8 parked. Held items (R1 debris, I1–I3 infra, F1–F5 taste) remain Kevin-gated.
- **`fc26af1` B6 — humble starting loadout (loot/craft loop matters).** Stripped the trivializing starter kit (was Diamond Sword + near-full armor + diamond/gold ore hoard + 5+5 potions → nothing to earn/craft turn 1). Now: building basics + basic tools + ONE Stone Sword + a little bootstrap ore (first craft reachable, not gifted) + a 2+1 potion safety net. Iron/Diamond gear is crafted (data/recipes.js) or looted; diamond/gold ore mined. Locked by `tests/store/startingLoadout.test.js` (6 tests). The DOM hotbar renders inventory.blocks → the reduced loadout legitimately changed the world-frame visual baselines (re-baselined).
- **Bonus (required to land B6's re-baseline): fixed a pre-existing capture day/night NON-DETERMINISM.** The 3 panel frames (inventory/achievements/progression) rendered the live day/night world behind their translucent backdrop, flaking day-vs-night run-to-run (11–33% phantom diffs that NO setTimeOfDay/isDay pin resolved — diagnosed: not the clock [4 units/s can't cross a 600 half-cycle in the ~6s panel section], not isDay; the world-render itself). Since the panel UI is the regression target, `capture.mjs` now hides the 3D canvas for those 3 shots → deterministic solid backdrop (DOM HUD intact). **Verified green across two consecutive gate runs** (ends the loop's "21 green" luck-dependence — now actually deterministic). Gates: 1685 unit / 21 visual ×2 / build + eslint.
- **Phase-B queue remaining:** B2 spell-color unify · B7 wand/crystal economy · B4 quest rewards · B3 shrine model · B1 flora slice-3 shapes · B5 mob/boss art. (B8 parked.)
...
### June 20, 2026 — ✅ PHASE A COMPLETE (entire code-review backlog + knip + kevinDecisions cleared)
- The 'fix everything' phase of Kevin's 2026-06-20 mandate is DONE. From the 39-agent exhaustive review (157 raw/156 confirmed): **5 HIGH bugs** + **~30 mechanical autonomousFixQueue** items (memory-leak disposes, dead-code purges, useShallow re-render cluster, NaN/clamp guards, per-frame alloc hoists, robustness guards) + **4 kevinDecisions implemented** (de-aggro leash, tree-through-rock, boss double-kill idempotency, save-data validation) + **2 surfaced** (BlockParticleSystem debris [verified rapier-2.2 API, fix ready, held for live-verify], taste set) + **knip 37-export backlog cleared to ZERO** (ignoreExportsUsedInFile + 2 dead deletes).
- **`ddd62b5` impact-ring dedup (last Phase-A item):** spell-projectile hits rendered two stacked rings (damageMob's ImpactShockwave + the spell's SpellImpactPop). Added a `spawnRing` param to damageMob (default true -> melee/hurl/zone keep their ring); the spell-projectile direct-hit passes false (owns SpellImpactPop). Melee untouched — mobdeath-probe LIVE-LOOK confirms one clean ring + death burst. kill-attribution gate made extension-tolerant. Spell-side single-ring look flagged for Kevin's eye (headless can't cast).
- ~30 gated commits this session (5b85cb..ddd62b5), each build+1669 unit+eslint green, pushed to main (auto-deployed). Process lessons banked: chain gates so a fail BLOCKS commit; git from repo root via -C; `>|` + /dev/stdin to beat zsh noclobber.
- **NEXT = PHASE B** (the 'build everything' phase): deferred KEVIN-REVIEW features as proper milestones (biome-flora render-wiring, spell-color unify, distinct shrine model, quest gear/coin rewards, mob/boss art, starting-inventory rebalance, wand/crystal economy) + SOTA-INITIATIVE.md remaining. S4 multiplayer/monetization = surface scope-confirm first.
...
- **Exhaustive review** (`db81098` report = `docs/superpowers/CODE-REVIEW-2026-06-20.md`): 39-agent workflow, every source file line-by-line → adversarial verify → synthesis. **157 raw / 156 confirmed** (1 false-positive dropped; 5 true-HIGH, 9 demoted). Codebase broadly well-architected (GLI/capture-determinism/resource-hygiene consistently correct); defects cluster in prop-attached three.js resources missing dispose, un-throttled per-frame work, lifecycle edge cases.
- **4 of 5 HIGH bugs fixed (gated + pushed):** boss Phase-1 fireball applyEuler-on-world-vector → wrong origin + frequent zero-damage, + fireball/lava mesh-map leak (`4b4ba75`); static hub NPCs distance-culled with no respawn (trading/smithing/healing lost after roaming) + leap-knockback NaN guard (`e5df50f`); **Terrain ChunkMesh Rules-of-Hooks early-return that HARD-CRASHED the render tree** on an empty↔non-empty chunk change (now reachable via ocean/water mining) (`19475ba`); frozen mobs stuck at 40% slow forever after the last ice zone expired (`a28153d`). 1669 unit / build / eslint green throughout.
- **Remaining:** HIGH #3 (worker `update_block` drops `grassTops` → wind-grass vanishes on block edit; deferred — locked worker, needs gate-token check) + 36 mechanical autonomousFixQueue items (dispose/leak/dead-code/useShallow/NaN-guards/alloc-hoists) + the knip backlog (37) + impact-ring dedup. **17 Kevin-decision items** (de-aggro leash, grantXP full-heal, save-data validation, HUD a11y/innerHTML, BlockParticleSystem rapier migration needing live render-verify, etc.) in the review doc.

### June 20, 2026 — Codebase-tooling evaluation (Kevin directive) + knip wired (`d5b85cb`)
- Kevin asked for a deep/holistic eval of **codebase-memory-mcp** + **brooks-lint** + a workflow sweep for SOTA alternatives, esp. for Crafty. Ran an **8-agent workflow** (`wf_b6a24799`): deep-dive each candidate → adversarial skeptic each → SOTA-landscape sweep of both categories (code-memory MCP + AI-review/lint) → a read-only profile of the real Crafty codebase → synthesis. All claims live-verified (`gh api` + multi-engine search + grep of our repo).
- **Verdict — SKIP both named tools as autonomous-loop fixtures** (both adversarial verifiers = skip/HIGH; both deep-dives 3/5). **codebase-memory-mcp:** `trace_path` verified broken on macOS-arm64 v0.8.1 (#480 OPEN); `create(()=>({...}))`/hook internals not indexed (#415 OPEN — confirmed `useGameStore` line 48 is exactly that shape, so ~all store actions are invisible to its graph); silent-index-corruption-as-success (#391/#333) is categorically unsafe for a loop that trusts output; 757 commits/14d ⇒ perpetually-stale graph; ad-hoc-signed single-maintainer C binary + auto-config-rewrite vs sovereign config; README inflates the peer-reviewed paper (120×→10× tokens, 158→66 langs, hides 83%-vs-92% answer-quality drop). **brooks-lint:** negative token ROI (~1.2k resident/session + ~12k scaffolding/run) on a token-sensitive loop; ZERO React/R3F awareness ⇒ guaranteed FP flood vs 40 `useFrame` files + 144 `useEffect` + the god-files; benchmark measures parser format-fidelity not detection accuracy; auto-fix unsafe on working game code; value matched by built-in `/code-review` at zero resident cost.
- **Acted autonomously:** wired **knip** (`d5b85cb`) — the deterministic dead-code detector the project ALREADY configured (`.knip.json` present) but never installed; added `knip@6` devDep + `npm run knip` (report-only) + made `.knip.json` test/scripts-aware (the src-only config false-flags test-only consumers). knip is local, React/Vite/Vitest-aware, ground-truth (cannot hallucinate reachability), near-zero agent-token cost. First clean run = **54 unused exports + 1 unlisted dep** ⇒ a follow-up cleanup backlog (mix of redundant re-export facades, internal-only over-exports, and genuinely-dead symbols — each needs un-export-vs-delete judgment + static-gate string-match check, so worked in careful gated batches, not one mass-delete).
- **Discipline note:** the workflow agent claimed the `getItemRarity` re-exports were dead ("no consumer"), but that was a src/-only observation — my grep of src/ **AND** tests/ found `tests/data/loot-characterization.test.js` imports them. Re-confirmed the banked "grep src/ AND tests/" lesson; will trust knip's deterministic graph (now test-aware), not an agent's grep.
- **For Kevin (KEVIN-REVIEW):** install an MCP server? (best nav tool = oraios/serena LSP + ast-grep, both local/MIT — needs your OK, hand-wired `.mcp.json` scoped to frontend/ only). Any cloud-LLM reviewer = privacy call (in-session `/code-review` is the local default). `npm audit` bump pass? Standing rec: keep all auto-fix/deletion report-only in the loop; stay fully local; skip both named candidates.
...
- All on `main` + auto-deployed; 1663 unit / 21 visual / build + eslint green throughout. **#5 (dead 'wand' currency) reclassified Kevin-gated** — removing it orphans crystals (their only sink); it's an economy-balance call (see KEVIN-REVIEW).
- **Also shipped (sprint cont.):** **`844cbdf` touch minimap** — mounted the radial minimap on touch (was desktop-only; touch players had no map), repositioned top-right; touch-probe LIVE-LOOKed clean; capture-neutral (minimap null in capture). **`e1f4578` over-exports** — dropped `export` on 4 module-local-only symbols (kept MOOD_SCALARS + OCEAN_FULL_SPAN — test-consumed; reverted an items.js Title-Case that broke a test for zero player value). **`97b7991` agents-doc** — fixed stale `.agent/AGENTS.md` (deleted test_swarm.js claim + drifted god-file LOC). **`a826330` dead synth voices** — removed `magic`/`magicCharge` voice builders + VOICES entries (0 trigger callers; live magic SFX = magicCast/Hit/Explosion). **`ee92c1e` orphan sound wrappers** — removed playMagic/playMagicCharge/playJump/playTone (0 consumers). **Banked lesson:** grep BOTH src/ AND tests/ before any delete/un-export (tests are consumers — my src/-only grep missed two, caught by the gate). Continuing: stale comments, then chain-lightning visible arc + impact-ring dedup; iceball-freeze + the rest are Kevin-gated.

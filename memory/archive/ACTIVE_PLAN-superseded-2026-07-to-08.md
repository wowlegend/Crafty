# ACTIVE_PLAN — superseded cursors, 2026-07-13 → 2026-08-08

> **Archive, not history-of-record.** `memory/CHANGELOG.md` is the history and `docs/superpowers/DECISIONS.md`
> is the decision record; this file exists only because `memory/ACTIVE_PLAN.md` accumulated SEVEN stale
> cursors (back to 2026-07-13) while its own preamble said it owns THE CURSOR ONLY and must stay short.
> Moved here rather than deleted, per archive-over-delete. Nothing below is current. Do not mine it for
> "what is next" — a stale plan is a live trap; the live cursor is in `memory/ACTIVE_PLAN.md`.

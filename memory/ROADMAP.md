...
> ⚠️ **Superseded framing (2026-05-31):** the "Phase 19–25" roadmap below is the pre-initiative Gemini plan. The ACTIVE roadmap is now the **Crafty → SOTA initiative**, decomposed into streams: **S0** reality audit (DONE) · **S1** art direction (locked; **S1-A foundation DONE** · **S1-B render recipe DONE** — M1 render correctness + device tiers, M2a mood/atmosphere + bright-Caribbean art, M2b character render language [2-band toon + fresnel rim + inverted-hull outlines; boss emissive telegraph preserved]; all merged to `main`, `test:visual` 6/6) · **S1-C UI design system IN PROGRESS** — **M1 token-foundation + bold-flat primitives + i18n DONE** (2026-06-01, merged; tokens→CSS-vars→Tailwind SoT chain, 9 primitives (read `src/ui/primitives/index.js`), en/zh i18n with lazy-CJK, `test:visual` 8/8); **M2 consolidate-3-languages + UI-hex-migration NEXT**, then M3 icon-system + emoji-data-decouple; then S1-D signatures (Spell-VFX, Mascot) · **S2** game design · **S3** engine (touch input DONE 2026-06-15 · de-monolith PARTIAL [SimplifiedNPCSystem + GameScene dissolved; **5 files are still >=900 LOC** — re-measure with `node frontend/scripts/ci/measure.mjs`, do not trust a number here] · remaining: ECS hardening, WebGPU/TSL migration) · **S4** multiplayer + monetization. See `SOTA-INITIATIVE.md` + `docs/superpowers/` (specs + per-milestone plans). Items below are retained as historical/backlog reference.

*(Header stamp REMOVED 2026-08-13: it read "June 11, 2026" on a file last written 2026-08-13 and containing a section dated that day. A date typed by hand at the top of a living file is a claim nothing checks — read `git log -1 -- memory/ROADMAP.md` instead.)*

> **STATUS (2026-06-15, HEAD eb924db) — ALL SELF-VALIDATABLE FRONTS DONE; REMAINING WORK IS KEVIN-GATED:** This session shipped (all on main, pushed): (1) P0 camera fix — own src/input/pointerLook.js + first LIVE-input gate npm run test:look (scripts/visual/look-e2e.mjs), replacing element-fragile drei PointerLockControls; (2) music overhaul — ElevenLabs day/night/boss tracks + crossfade via src/ui/MusicPlayer.jsx, procedural pad/arp muted (PROC_MUSIC_GAIN=0); (3) terrain flattened + sun mesh shrunk (radius 24->13); (4) TOUCH FRONT COMPLETE (probe-exhausted) — iOS cold-start bridged via enterPlay() (MenuSystem.jsx), QuestTracker collapses, spell-label nowrap, compact SimpleExperienceBarTouch readout, deterministic scripts/visual/touch-probe.mjs; minimap JUDGED+SKIPPED; (5) OCEAN MILESTONE COMPLETE (plan docs/superpowers/plans/2026-06-14-crafty-ocean-coast.md, 4 slices S1-S4: tropical teal + sheen, shore foam via un-merged water-top faces, depth-graded top surface, ocean-coast capture state; new scripts/visual/ocean-probe.mjs). **Gates (2026-06-15 SNAPSHOT, since superseded): 1182 unit / 20-state visual. Now 3189 unit / 31-state — re-measure, do not quote.** **ONLY REMAINING WORK = KEVIN-GATED (KEVIN-REVIEW #37/#38):** redeploy crafty-sand.vercel.app, real-device iOS touch feel, music ear-check, ocean taste sign-off; plus OPTIONAL sun-arc / menu-music. Loop in ~45min steady-state hold. The STATUS blocks below are HISTORY (newest first).
...
> 23 gates / visual 13/13 [the inline correction "now 1182 unit / 20-state" has ITSELF gone stale — 2026-08-13 measures 3189 unit / 31-state. Two generations of stale correction in one line is the argument for naming the command, not the number.]; zero re-baselines across B3+B4). ALSO SHIPPED 2026-06-10/11: Kevin's
...
- [x] **Dynamic Combat Arpeggiators**: Synchronize chord arpeggiations and synthesizer tempos to hostile population. **NOW INAUDIBLE** — `SoundManager.jsx:14` sets `PROC_MUSIC_GAIN = 0` and every arpeggiator gain ramp multiplies by it; ElevenLabs tracks replaced it (`ui/MusicPlayer.jsx`). Built, shipped, then muted: still [x], but nobody can hear it.
...
reason: it never gets closer. See `memory/ACTIVE_PLAN.md` (the pinned-postprocessing block), the 2026-08-13 CHANGELOG entry, and upstream **pmndrs/postprocessing#750**. NOT `.claude/rules/` — I wrote that pointer on 2026-08-13 and nothing there documents the sun/GodRays contract.
...
- [x] **Dynamic Infinite Spawner Loop Engine**: Re-engineered the SpawnerSystem tick — now a **1000ms** useFrame throttle in `src/systems/SpawnerSystem.jsx` (extracted from SimplifiedNPCSystem.jsx in the v6 de-monolith A1.3). Replaced chunk-specific Set tracking with an active-mobs population cap check (max 16) that dynamically spawns mobs inside a ring range of `[28, 85]` blocks inside active generated chunks around the player.
...
- [ ] **Persistent World Saving**: ~~[x] backend support for chunk compression and saving player states directly to the database~~ — **NEITHER HALF EXISTS.** Persistence is localStorage-only (`game/worldSaves.js`, "local-first"), and there is no backend or network call anywhere in `src/`. Un-ticked 2026-08-13: a [COMPLETED] box on unbuilt work is worse than an open one.

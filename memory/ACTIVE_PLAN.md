...
re-capture. `baseline-trailer` forbids bundling that rewrite with the `src/` change that caused it, so it is
two commits per item, and `npm run test:visual` is manual and takes ~12 minutes. Budget for it; do not start
one at the end of a session.

**ALSO PENDING, and it is a real debt:** `ec57fa4` (the clock's two origins) MOVES the compass dial in every
HUD-bearing baseline — day frames from 90° (sunset) to 0° (noon), night frames from -90° to 180°. Both are
corrections. The baselines have NOT been re-captured yet, so the visual oracle is stale in exactly that
region. That re-capture is owed and must carry a `Baseline-Review:` trailer.
...
**AWAITING KEVIN (taste, not bugs):** the ice beast's rim reads salmon rather than cyan under the warm
magic-hour grade — the code anticipates this and boosts cool rims; his call whether it is enough. Also the
touch-ring left-facing-arc ergonomics, and the grass motes (deleted, veto-able).

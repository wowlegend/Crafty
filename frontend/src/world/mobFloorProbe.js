...
/**
 * (x, z, feet, clearance?) -> the floor under a mover at those feet | Infinity (the column is a wall to it) |
 * null (no data). `clearance` defaults to a mob's; floorUnderPoint passes 0 for a point.
 *
...
export function makeMobFloorProbe(rapier, world) {
  ...
  return (x, z, feet, clearance) => {
    ...
    return floorInColumn(columnFaces(castDown, feet), feet, clearance);
  };
}

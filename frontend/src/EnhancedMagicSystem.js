...
export const EnhancedMagicSystem = ({ gameState, playerPosition }) => {
  ...
  const applyBurnEffect = (mobId, duration, dps) => {
    ...
    const burnInterval = setInterval(() => {
      ...
      console.log(`🔥 Burn tick: ${dps} damage to mob ${mobId}`);
      ticksRemaining--;
    }, 1000);
  };
  ...
  const applyFreezeEffect = (mobId, duration, slowPercent, freezeChance) => {
    ...
    if (Math.random() < freezeChance) {
      ...
      console.log(`🧊 Mob ${mobId} is COMPLETELY FROZEN for 2 seconds!`);
    }
    ...
    for (let i = 0; i < maxChains; i++) {
      ...
      if (nearestMob) {
        ...
        if (window.damageMob) {
          ...
          console.log(`⚡ Chain ${i + 1}: ${Math.floor(currentDamage)} damage to ${nearestMob.type}`);
        }
  ...
  useEffect(() => {
    window.castSpell = (spellType = 'fireball') => {
      ...
      if (window.useMana && !window.useMana(manaCost)) {
        console.log(`❌ Not enough mana to cast ${spellType}! Need ${manaCost} MP`);
      ...
      // Then sync state for React rendering
      setProjectiles([...projectilesRef.current]);

      console.log(`🔥 Cast ${spellType}! Damage: ${finalDamage} (${damageMultiplier.toFixed(1)}x multiplier), Cost: ${manaCost} MP`);
    ...
    const updatedProjectiles = projectilesRef.current.map(projectile => {
      ...
      if (window.checkMobCollision) {
        ...
        if (hitMob) {
          ...
          console.log(`🎯 ${projectile.type.toUpperCase()} hit ${hitMob.type}! Damage: ${projectile.damage}`);
          ...
          if (spellConfig?.secondary) {
            ...
            switch (sec.type) {
              case 'burn':
                ...
                console.log(`🔥 ${hitMob.type} is BURNING for ${sec.duration}s!`);
                applyBurnEffect(hitMob.id, sec.duration, sec.damagePerSecond);
                break;

              case 'freeze':
                // Apply slow effect
                console.log(`❄️ ${hitMob.type} is FROZEN/SLOWED by ${sec.slowPercent}%!`);
                applyFreezeEffect(hitMob.id, sec.duration, sec.slowPercent, sec.freezeChance);
                break;

              case 'chain':
                // Chain lightning to nearby mobs
                console.log(`⚡ Chain lightning jumping to ${sec.maxChains} targets!`);
                applyChainLightning(
                  projectile.position,
                  hitMob.id,
                  projectile.damage,
              ...
              case 'pierce':
                ...
                console.log(`💜 Arcane lifesteal: healing ${healAmount} HP!`);

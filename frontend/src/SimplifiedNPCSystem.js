...
export const NPCSystem = ({ gameState }) => {
  ...
  const spawnMob = (x, z, forceType = null) => {
    ...
    // Random mob type selection
    const mobTypeKeys = Object.keys(MOB_TYPES);
    const type = forceType || mobTypeKeys[Math.floor(Math.random() * mobTypeKeys.length)];
  ...
  useEffect(() => {
    ...
    const mobs = [];
    // Spawn variety of mobs in a ring around player
    for (let i = 0; i < 20; i++) {
    ...
    console.log(`✅ Spawned ${mobs.length} initial mobs with variety`);
  }, [terrainReady, camera]);
  ...
  useFrame(() => {
    ...
    if (!spawnedChunks.current.has(chunkKey)) {
      ...
      if (newMobs.length > 0) {
        ...
        console.log(`🐷 Spawned ${newMobs.length} mobs in chunk ${chunkKey}`);
      }
    }
    ...
    setEntities(prev => {
      const updated = prev.map(entity => {
        ...
        if (newEntity.moveTimer <= 0) {
          ...
          newEntity.isMoving = Math.random() > 0.3; // 70% chance to move
        ...
        // Move towards target
        if (newEntity.isMoving) {
          const dx = newEntity.targetX - entity.position[0];
          const dz = newEntity.targetZ - entity.position[2];
          const dist = Math.sqrt(dx * dx + dz * dz);

          if (dist > 0.5) {
            const speed = mobConfig.speed * delta;
            ...
            newEntity.rotation = Math.atan2(dx, dz);
          } else {
...
export const TradingInterface = ({ villager, gameState, onClose }) => (

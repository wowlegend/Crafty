...
> **✅ SHIPPED (2026-06-10, loop iter 63):** both tasks in one iteration. 786 unit (93 files) · build · visual 13/13.
...
- [x] Fix Components.jsx:808 `\`0,${y},0\`` → `\`0_${y}_0\`` (the writers' underscore shape).
- [x] Extend `tests/gates/kill-attribution-gates.test.js`'s sibling (or add to allegiance-gates): a key-shape gate — no `worldBlocks.has/get` call anywhere in src may use a comma-template key (regex over the god files; self-extending).
- [x] Battery + commit `fix(elemancer-m1): the comma-key worldBlocks always-miss (+ a key-shape gate)`

### Task 2: pin the 'hazard' source construction
- [x] mobKillBus.test: emit with 'hazard' → a 'player'-filtered handler (the ferocity shape) banks NOTHING; the docstring source enum gains 'hazard'.
- [x] The kill-attribution gate already enforces filtered === subscribers (the per-subscriber invariant — auto-covers any new source); add one explicit test naming 'hazard' so the contract reads in the suite.
- [x] Battery + commit `feat(elemancer-m1): the 'hazard' kill-bus source pinned — zone kills bank nothing by construction`

...
> **✅ SHIPPED (loop iter 108, 2026-06-13).** The plinth (`world/homeAnchor.js` `stampHomeAnchor`) + the `<HomeAnchorRender/>` voxelKit lodge/brazier landed; `HEARTH_Y` corrected 32→56 after visual verification caught the ~Y50-origin pit bug; a dedicated `hearth` capture fixture added for real gate coverage. Commits `9e3...`(T1)/`...`(T2)/`82d3523`(fix+fixture). 886 unit · build · visual 14/14. Deferred to fast-follows: the lodge-richness polish + the pier (post-M2 water). NEXT = M2 (ocean depth + coastline consts).
...
> **⚠️ BUILD CORRECTION (iter 108 — supersedes `HEARTH_Y=32` in the code blocks below):** the draft assumed origin natural height "~Y30" (per the spec draft). Verified WRONG against the real worldgen formula (`30+n*40`, `createNoise2D(lcg(12345))`): origin surface is **≈Y50** (footprint [-7,7]²: min 41 / max 58 / avg 49.5; ring 37–61). `HEARTH_Y=32` carves a sunken pit. **Built value = `HEARTH_Y=56`** (and `HEARTH_TOP=56` in Terrain.jsx) — a gently-raised crafted pad over the ≈Y50 grade, high enough to land in the diorama capture frame. The flatten LOGIC and all tests are unchanged (parameterized on `HEARTH_Y`); only the constant differs from the snippets below.

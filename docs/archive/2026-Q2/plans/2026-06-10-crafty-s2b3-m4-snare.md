...
> **✅ SHIPPED* (2026-06-10, loop iters 44-46):** T1-T3 live (intent/KeyX · the SM block + bind apply-site + chime · the tether + tell). 765 unit (88 files) · build · visual 13/13. ***The asterisk:** the tether MECHANISM is numerically verified live but the clean AESTHETIC judge frame is parked after 6 framing attempts (writeup: `.superpowers/s2b3-soulbind-m4-refs/HOW.md`) — folded into M6's look-judge milestone, where SOULBIND's full look (tether + tell + ally tint + fusion silhouettes) gets judged together.
...
- [x] **Step 1 (red):** extend the inputState test — INTENT_KEYS contains 'snare'; setIntent('snare', true) reads back; reset clears it (twin the 'grab' lines exactly).
- [x] **Step 2:** `INTENT_KEYS = [..., 'roar', 'grab', 'snare']`. In Components keydown beside KeyV: `if (e.code === 'KeyX') { setIntent('snare', true); }` (+ the keyup false twin). Comment: `// S2-B3-M4: snare -> the abstract 'snare' intent (SOULBIND SM in useFrame). The Aspect-verb row: R=roar, V=grab, X=snare.`
- [x] **Step 3: green + commit** `feat(soulbind-m4): the snare intent on KeyX (the Aspect-verb row R/V/X)`
...
- [x] **Step 1:** `snareChannel.js` — the transient render channel (hurlChannel's sibling, TDD):
```js
/**
...
- [x] **Step 2:** the SoundManager 'bind' chime — a 2-note rising jade resolve in the grab-chirp family (whole-tone rise that LANDS — binding completes):
```js
...
- [x] **Step 4: full battery + commit** `feat(soulbind-m4): SNARE plays — channel, bind, Soul debit, capture, jade tint, chime`
...
- [x] **Step 1:** SnareTetherSystem — ONE always-mounted line-ish tether driven transiently (the impact-flash pattern; NO new lights): a thin stretched cylinder (or a 2-point ProceduralRibbonTrail if its API fits — READ Components :1260/:1496 first) from `readSnareState().from` to `.to`, jade `#3DFFB0`, opacity = 0.25 + 0.55 * progress (the channel VISIBLY tightens), `visible=false` whenever `!channeling` or `isCaptureMode()`.
- [x] **Step 2:** the tell — in the MobModel floating health bar (locate the bar's color/material), tint the fill jade when `entity.health <= 0.3 * entity.maxHealth && !entity.passive` (an honest "weakened — bindable" read; static under capture since health is static).
- [x] **Step 3:** in-world judge (the charter look-gate): drive a real snare — dev console: damage a spawned hostile to <30% via `GameMethods.damageMob(id, dmg)`, hold X aimed at it, screenshot mid-channel (the ?perf=C camera-lock method if the live SM fights it — the tether reads from the transient channel, so a synthetic `writeSnareState({...})` burst is the deterministic fallback for FRAMING only; the REAL channel smoke must also pass via __hurlDebug-style observation or the store/alliesQuery delta). Judge: does the tether read as a soul-ribbon (not a laser)? Does the jade bar tell? Iterate once if weak (the M7-T2 tune precedent).
- [x] **Step 4: full battery** (visual 13/13 — the tether is capture-nulled) **+ commit** `look(soulbind-m4): the jade tether + the weakened tell (judged in-world)`

### Task 4: close-out
- [x] Spec §3 M4 row ✅ · this plan ✅ SHIPPED · KRB playtest cue (weaken a zombie → hold X → watch the tether tighten → the bind chime + jade creature follows... no following until M5 — say so honestly: "it stands guard where bound until M5") · ACTIVE_PLAN → M5 (squad AI: the pure brain + the 15Hz bridge + AllyModel render).

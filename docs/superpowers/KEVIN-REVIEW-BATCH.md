...
> **🎭 S1-D MASCOT — Kevin's taste call (3 concept directions; I'll render one reference each, same camera/light, when we reach M4):** **(A) "Spark Familiar"** [my recommended lead] — floating voxel elemental companion, tint-shifts to your active spell, ember aura via the GPU spark system; low-rig, high cosmetic-SKU + strong Marcus appeal, cross-wired to the spell signature. **(B) "Crafty Hero"** — chunky toy-like apprentice-mage avatar (evolves the current pointy-hat); most commercial/customizable but needs a humanoid rig. **(C) "Craft-Golem"** — friendly creature built from the world's own voxel blocks + a glowing rune-core (reuses the spell-emissive material); most theme-ownable. All ship on the existing toon+rim+outline pipeline; acceptance = shape-language + 100%-black-silhouette test. **Which direction (or a blend)?**
> **🎇 S1-D spell-VFX — FIRST FRAME to eyeball (`frontend/tests/visual/baseline/spell-cast.png`):** M1 spine + M2 cast-arc built (rune-circle telegraph → stretch-trail → fireball → ember spray; deterministic; 11th visual state). My honest read: the **spine is correct but the look isn't premium yet — the fireball is a flat pastel sphere, not a hot emissive/glowing fireball; telegraph + spray are faint.** (Caveat: it's a frozen single frame in a blank sky-studio — the live cast w/ motion + impact bloom-spike + shake reads better.) **Decisions for you:** (1) how much "wow"/energy to push (I'd reshape the fireball → emissive fiery core + bloom as the next polish); (2) the spell **shape vocabulary** (rune-circle [current default] vs hex-glyph) + per-element personality; (3) the **magic-hour color script** (premium-vs-"candy" band). Spine + atmosphere elevation (M3) proceed autonomously; the polish/personality awaits your steer.

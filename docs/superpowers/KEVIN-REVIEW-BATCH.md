...
> **🎇 S1-D spell-VFX + atmosphere taste-gates (batch when built):** the spell **shape vocabulary** (rune-circle vs hex-glyph) + per-element cast personality (2-3 rendered casts), and the **magic-hour color script** (premium-vs-"candy" band). Buildable spine (M1: route spell impacts through the existing GPU spark system + camera-shake + non-blocking hitstop, deleting the React-sphere slop) proceeds autonomously.

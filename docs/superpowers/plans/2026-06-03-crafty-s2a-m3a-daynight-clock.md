...
3-lens adversarial review = spec/quality/**determinism+game-loop-isolation** all APPROVE. One MINOR (spec+quality, consistent): `isDayAtUnit` + `CYCLE_UNITS` were exported+tested but had no non-test consumer, and the JSDoc claimed `isDayAtUnit` was a load-resync helper the load path didn't use. **Resolved in a fix-up commit** (NOT a deferral): (1) **wired `isDayAtUnit` into `loadWorldData`** — `isDay` is now derived from the restored `gameTime`, so a resumed save is always phase-consistent (the helper's promised purpose is now real; a save whose stored `isDay` disagreed with its `gameTime` no longer loads inconsistent); (2) **extracted the per-tick pause decision to a pure `shouldAdvanceClock`** so the load-bearing determinism + Game-Loop-Isolation contract is exhaustively unit-tested (7 cases) instead of only static-text-gated — a guard inversion now fails a behavioral test; (3) hardened the store-test `beforeEach` to reset via `setState` directly (order-independent; addresses the determinism reviewer's non-reproducible first-run flake). Skipped (YAGNI, unreachable): NaN-defensiveness on `crossedHalfCycle` (sole caller is the integer ticker from a numeric base). `test:unit` 478 · 12/12 visual after fix-up.

...
- 2026-06-16: **M2 attack telegraphs (#4) IN PROGRESS** — Slice 1 timing core DONE (`7a74a14`): pure `game/attackTelegraph.js` dodgeable windup->strike machine + AI-worker windup gate (~380ms) + windupUntil round-trip. 1310 tests / 20/20. Slice 2 render DONE (`fa60ee6`: coil pose + red-orange emissive charge ramp, windupRamp). 1317 tests / 20/20. NEXT: slice 3 boss AoE ground-ring (ground boss attack path first).
...
3. **[H/M/AUTO] Player-facing Settings** — Audio (master/music/SFX/mute) + Accessibility (reduced-motion, screenshake, colorblind, text-scale) + Controls remap; split master bus into music/sfx sub-buses; persist; read OS reduced-motion. (UX/A11Y)
4. **[H/M/AUTO] Enemy + boss attack TELEGRAPHS** — windup event ~350-400ms pre-strike in ai.worker.js + anticipation pose + element-emissive charge ramp + reserved red=unblockable + boss ground-ring lava warning. (the bar's flagship anti-pattern; FEEL timing is the Kevin-gated part). (MOBS/BOSS/COMBAT)
5. **[H/L/AUTO] Terrain-aware GPU wind grass** — replace the DEAD OptimizedGrassSystem; per-chunk instanced blades, slope-reject, density fade, player/mob bend, multi-octave wind, backlit. (the #1 "2019 hobby" tell). (WORLD/VEGETATION)
...
M1 game-feel core (#1) + the verified quick-win bug cluster (ore-debris, playCraft, pitch-jitter, npm-test). → M2 telegraphs (#4) + directional impact (#9) + death/victory beats (#7). → M3 Settings/A11y (#3) + input buffering/coyote/touch-dodge (#6). → M4 living world: wind grass (#5) + real landmarks (#8) + far-LOD (#18) + biomes (#12). → M5 systems depth: inventory correctness (#15) + element-zone VFX/beast verbs (#11) + mining feel (#13) + squad depth (#19). → M6 polish: lighting bounce (#14) + HUD aliveness (#10) + audio adaptive layer (#16) + pre-game brand (#17) + repo health (#20).

...
| R7.9 | column-top readers left after R7.1 | same root, other consumers: `EnhancedMagicSystem` impacts a projectile at `y <= columnTop + 0.5` (a spell cast under a tree or a roof should burst on launch — VERIFY in the running game first); XP orbs and loot (`xpOrbStepper` / LootSystem `groundYAt`) settle on the canopy of a mob killed under it; `spawnPlacement` can put a mob ON a canopy or roof | OPEN — verify, then a point query (clearance 0) on the same floor rule |

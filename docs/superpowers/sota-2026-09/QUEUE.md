...
| R7.9b | `SpawnerSystem.spawnMob` | spawn placement still takes the column top, so a mob can SPAWN on a tree canopy or a roof (it walks off the edge and drops — cosmetic, but Minecraft-genre players read it as a bug) | OPEN — spawn on the lowest roomy floor of the column instead |

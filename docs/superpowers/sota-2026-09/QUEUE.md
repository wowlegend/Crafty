...
| # | Where | Finding | Disposition |
|---|---|---|---|
| R7.1 | `game/localPath.js` settleOnGround | REGRESSION of R6.4: the own-column exemption accepts ANY rise, and the probe is top-down from y=255 — a roof or bridge built ABOVE a mob (air between) lifts it onto the roof | OPEN — HIGH. **Diagnosis (2026-09-23): the root is the PROBE, not the exemption.** Chunk colliders are TRIMESHES (`Terrain.jsx` TrimeshCollider) and getMobGroundLevel casts down from y=255, so 'ground' = the top surface. Under a roof, an overhang or a TREE CANOPY (leaves are in the collider) that is the wrong floor — so P1's step rule (232f0581/43154097) very likely also refuses mobs walking UNDER trees and roofs (to verify in the running game first). Design: a mob floor probe `getMobFloor(x, z, feetY)` — top-down gTop; if gTop - feet > STEP_UP, cast UP from feet + STEP_UP + 0.1: a hit near gTop = solid (a wall: refuse); a hit well below = a ceiling with air under it → the floor is a down-cast from just under the ceiling, passable if the gap ≥ mob height. Relies on trimesh rays hitting faces from inside/behind — VERIFY with @dimforge/rapier3d-compat in node (already a dependency via @react-three/rapier) before building. Grid heights and the knockback probe take the same probe. |

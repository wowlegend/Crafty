...
| R8.10 | `mobFloor.spawnGroundAt` | its refusal silently depends on SPAWN_SOLID_DEPTH + 0.1 <= FLOOR_REACH; past it every canopy reads as ground | OPEN — pass its own reach |
| R8.11 | `chunkLayout.chunkOrigin` | (self, found at commit) exported with zero callers | OPEN — finish it: the `c * CHUNK_SIZE` sites |

...
| **P5** | **Broad-audience (≈8→young-adult→adult) accessible + fair monetization — legible, joyful, zh-CN-ready, and NEVER pay-to-win, even over** hardcore-gatekeeping that locks out newcomers OR power-monetization for max ARPU | A new verb is graspable in **<60s** (good UX at every age — "easy to learn, hard to master"; NOT dumbed-down depth) + has a zh-CN string path + monetization sells **cosmetics/VFX only, never gameplay power/advantage/progression** (= never-sells-power / no pay-to-win / no gacha). **Marcus (8yo) is A user, not a depth-lowering ceiling** — the lines are blurred toward a wider teen/adult audience, so intensity, real stakes, even permadeath-*modes* are a DESIGN CHOICE, not vetoed. |

**Conflict resolution (deterministic):** **VETOES = {P5 no-pay-to-win (legal + brand-trust — the only hard part of P5), P4-hard-invariants (no mid-combat re-mesh; intent-abstraction not pointer-lock)}** → then ranked supports **P1 = P2 (core) > P3 > P5-accessibility (legibility/zh-CN/joy — a strong support, NOT a depth-veto) ≈ P4-soft (perf budget)**. P0 routes to PARK, never participates in CUT. (A coherent-but-unshippable feature cannot be greenlit through a perf wall; but accessibility/legibility NEVER vetoes depth — the wider audience wants both.)
...
- **Default-KEEP under uncertainty; burden of proof on the cutter; tie → DEEPEN.** *(Exception: if the **P5 no-pay-to-win** sub-test is uncertain — "does this monetize gameplay power?" — default flips to DEEPEN/FLAG, not KEEP, given the legal + brand-trust asymmetry. Accessibility/legibility and ambition (P0–P4) keep default-KEEP — a less-legible-than-ideal verb for the wider teen/adult audience is a DEEPEN, never a cut.)*

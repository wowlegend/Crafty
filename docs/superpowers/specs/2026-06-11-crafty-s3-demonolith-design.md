...
| S3-M4 ✅ (the safe strips) | **AdvancedGameFeatures dissolve, part 1** | ✅ SHIPPED 2026-06-13: the 2 standalone panels → `ui/` + the 3 accrual hooks → `world/accrualHooks.js` (AGF 1397→1120 LOC; 2 trap-1 gate repoints). **The stateful cores (boss/pet/survival SYSTEMS + render comps + the boss indexOf gate = trap 3) remain → S3-M4 Part 2.** | Medium safety; pattern 3×-proven. Part 1 = the lowest-risk surface (panels capture-suppressed, hooks self-null). |

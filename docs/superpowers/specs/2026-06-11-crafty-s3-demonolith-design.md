...
| S3-M4 (multi-part) | **AdvancedGameFeatures dissolve** | ✅ **Part 1** (2026-06-13): 2 panels → `ui/` + 3 accrual hooks → `world/accrualHooks.js`. ✅ **Part 2** (2026-06-13): survival domain (useSurvivalMode → `world/survivalSystem.js`, SurvivalWarning → `ui/`) + useSpellUpgrades → `world/spellUpgrades.js`. **AGF 1397 → 994 LOC**; 3 trap-1 gate repoints; all capture-clean. ⏭ **Part 3 = pets** (usePetSystem→world/, PetIndicator→ui/, PetEntities→render/; verify the allegiance-gates file-list). ⏭ **Part 4 = boss** (BOSS_CONFIG/useBossSystem/BossHealthBar/BossEntity — ⚠️ TRAP 3 the indexOf capture-anchor + the gated boss-obsidian/boss-closeup baselines → dedicated, adversarially reviewed). | Lowest-risk-first: parts 1-2 = the capture-clean surface; boss (the baseline-touching, indexOf-pinned core) is the reviewed finale. |
